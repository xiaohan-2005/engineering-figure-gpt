...
- `image` mode uses Codex built-in GPT image generation when available, with an official OpenAI GPT Image CLI fallback for reproducibility.
...
Inside Codex, the preferred path is the installed built-in image-generation capability. For portable/reproducible execution, `scripts/generate_image.py` provides an OpenAI-only fallback that defaults to `gpt-image-2` and intentionally rejects third-party base URLs.
...
The CLI fallback uses only the official OpenAI endpoint.
...
The CLI accepts `OPENAI_API_KEY`, `OPENAI_API_KEY_FILE`, or the local default key file at `~/.codex/secrets/openai_api_key.txt`. Never commit a real key to the repository.
...
7. Outputs should remain reproducible whenever possible.
