...
from image_model_policy import is_gpt_image_2, source_preserving_gpt_image_2_size
...
def resolve_edit_canvas(args: argparse.Namespace) -> int:
    """Preserve the primary source canvas by default for GPT Image 2 edits."""
    ...
    if not is_gpt_image_2(model):
        if args.mode == "correct":
            ...
            return 2
        args.size = "auto"
        print(f"[WARN] Model '{model}' is not GPT Image 2; edit output size is provider-controlled (auto).", file=sys.stderr)
        return 0

    try:
        size, exact = source_preserving_gpt_image_2_size(args.input_image)
    except Exception as exc:
        print(f"Could not resolve GPT Image 2 edit canvas from source image: {exc}", file=sys.stderr)
        return 2
    args.size = size
    if exact:
...
def add_image_runtime_options(parser: argparse.ArgumentParser, include_input_images: bool = True) -> None:
    ...
    parser.add_argument(
        "--input-fidelity",
        choices=("low", "high"),
        help="Only for models/endpoints that support it. GPT Image 2 rejects it because image inputs are always high fidelity.",
    )
    ...
    parser.add_argument("--final", action="store_true", help="Alias for final/high-resolution model routing.")
...
def output_paths_from_stdout(stdout: str) -> list[str]:
    ...
    for line in stdout.splitlines():
        ...
        if not text or text.startswith("{") or text.startswith("["):
            continue
        if Path(text).suffix.lower() in RASTER_SUFFIXES:
            paths.append(text)
    return paths
...
def cmd_image(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="efg-prompt-") as tmp:
        ...
        if args.figure_template:
            ...
            raw = Path(args.background_file).read_text(encoding="utf-8") if args.background_file else args.background
            if not raw:
        ...
        return run_image_and_verify(args, [str(SCRIPTS / "generate_image.py"), "--prompt-file", str(prompt_path), *runtime])
...
def cmd_edit(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="efg-edit-") as tmp:
        ...
        return run_image_and_verify(args, [str(SCRIPTS / "generate_image.py"), "--prompt-file", str(prompt_path), *image_args, *runtime])
